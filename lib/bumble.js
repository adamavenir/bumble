...
exports.register = function (plugin, config, next) {
    ...
    handlers = new Handlers(config, function _handlersLoaded() {
        ...
        servers.route({
            method: 'get',
            path: home + 'api/{year}/{month}/{day}/{pslug}',
            config: {
                handler: handlers.blogPostJSON,
        ...
        servers.route({
            method: 'get',
            path: home + 'api/{datePart*}',
            config: {
                handler: handlers.indexJSON,
                description: 'blog index JSON',
                tags: ['bumble', 'api']
            }
        });
        servers.route({
            method: 'get',
            path: home + 'api/tag{s?}/{tag*}',
            config: {
                handler: handlers.tagIndexJSON,
                description: 'tag index JSON',
                tags: ['bumble', 'api']
            }
        });
        servers.route({
            method: 'get',
            path: home + 'api/author{s?}/{authorSlug*}',
            config: {
                handler: handlers.authorIndexJSON,
                description: 'author index JSON',
                tags: ['bumble', 'api']
            }
        });
