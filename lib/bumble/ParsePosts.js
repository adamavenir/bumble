...
ParsePosts.prototype.parseFile = function (file, done) {
    ...
    fs.readFile(postDir + '/' + file.name, 'utf8', function (err, data) {
        ...
        postData.tags = map(postData.tags.split(','), cleanTag);
        postData.lastTag = last(postData.tags);
    ...
    function parsePostData(file, name) {
        ...
        lines.forEach(function (line) {
            if (done) {
                var lineWords = line.split(/\s/).length;
                // tests to see if the total length so far is less than the max intro words
                if (postWords < self.config.introPostWords) {
                    intro.push(line);
                }
                post.push(line);
                postWords = lineWords + postWords;
            } else if (!in_metadata) {
                if (line.match(/^---/)) {
                    in_metadata = true;
                } else if (line.match(/^\# /)) {
        ...
        each(postData, function (value, key) {
            if (value === "false") {
                postData[key] = false;
            }
            if (value === "true") {
        ...
        if (typeof postData.callToAction === 'string') {
            postData.callToAction = marked(postData.callToAction);
        } else if (postData.callToAction !== false && typeof self.config.callToAction === 'string') {
            postData.callToAction = marked(self.config.callToAction);
        } else {
            postData.callToAction = '';
        }

        return defaults(postData, {
            originalText: post,
            introBody: marked(intro),
...
ParsePosts.prototype.setup = function (config) {
    ...
    walker.on("end", function () {
        ...
        watcher.on('change', function (filename) {
            ...
            var postIndex = posts.indexOf(find(posts, { fullSlug: file.replace(/\.md$/, '') }));
