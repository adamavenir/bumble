...
ParsePosts.prototype.parseFile = function (file, done) {
    ...
    fs.readFile(postDir + '/' + file.name, 'utf8', function (err, data) {
        ...
        postData.fullSlug = postData.date.format('{yyyy}-{MM}-{dd}') + '-' + postSlug;
        postData.url = self.config.blogHome + postData.date.format('{yyyy}/{MM}/{dd}/') + postSlug;
    ...
    function parsePostData(file, name) {
        console.log('parsePostData', name);
        var postData = {};
        var post = [];
        var metadata = [];
