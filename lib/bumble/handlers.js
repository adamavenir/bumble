...
Handlers.prototype.latestJSON = function index(request, reply) {
    this.views.blogIndex(request.query.page, request.params.datePart, function (err, apiData) {
        ...
        // reply(apiData, {contentType: 'application/json'});

        var r = {
            page: 'page',
            date: 'date'
        };
        reply(r, {contentType: 'application/json'});
    });
};

Handlers.prototype.indexJSON = function index(request, reply) {
    var response = this.views.blogIndexAPI(request.query.page, request.params.datePart);
    reply(response, {contentType: 'application/json'});
};

Handlers.prototype.tagIndexJSON = function tagIndex(request, reply) {
    reply(this.views.tagIndex(request.query.page, request.params.tag, request.params.s), {contentType: 'application/json'});
};

Handlers.prototype.authorIndexJSON = function authorIndex(request, reply) {
    reply(this.views.authorIndex(request.query.page, request.params.authorSlug, request.params.s), {contentType: 'application/json'});
};

Handlers.prototype.blogPostJSON = function blogPost(request, reply) {
    // var response = this.views.blogPost(request.params.year, request.params.month, request.params.day, request.params.pslug);
    var response = {
        year: request.params.year,
        month: request.params.month,
        day: request.params.day,
        slug: request.params.pslug
    };
    this.views.rss();
    reply(response, {contentType: 'application/json'});
};
