...
function Views (config) {
    ...
    logger.info('config.blogAuthorEmail', config.blogAuthorEmail);

    function loadPosts(posts) {
        logger.info('starting loadPost with postDir', config.postDir);
    ...
    parsePosts.on('ready', function (posts, config) {
        logger.info('parsePosts ready with postDir', config.postDir);
        loadPosts(posts);
    });

    parsePosts.on('update', function (posts, config) {
        logger.info('parsePosts update with postDir', config.postDir);
        loadPosts(posts);
    });

    logger.info('parsePosts setup with postDir', config.postDir);
...
Views.prototype.blogYearIndex = function (req, res) {
    ...
    logger.info('views.blogYearIndex');
    var year  = req.params.year,
        month = req.params.month,
        day   = req.params.day,
        posts = new Array();
  
    console.log(this.postData, 'logging post data');
