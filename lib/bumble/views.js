...
function Views(config, done) {
    ...
    function loadPosts(posts) {
        ...
        _.each(posts, function (thisPost) {
            ...
            thisPost.matchTags = _.filter(self.postData, function (post) {
                //Tags match, not this post
                return _.intersection(thisPost.tags, post.tags).length > 0 && post.permalink !== thisPost.permalink;
            });
...
Views.prototype.blogPost = function (year, month, day, slug, next) {
    ...
    if (thisPost) {
        ...
        if (config.randomRelated) {
            relatedPosts = _.sample(_.shuffle(thisPost.matchTags), config.maxRelated);
        } else {
