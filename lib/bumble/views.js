...
//Express views
function Views(config, framework) {
...
Views.prototype.tumblrRedirect = function (req, res) {
    logger.info('views.tumblrRedirect', req.params, this.postData);
    var post = _.findWhere(this.postData, {slug: req.params.tslug });
    if (!post || !post.url) {
        return this.notFound(req, res);
    }
    if (this.framework === 'hapi') {
        return res().redirect(post.url).code(301);
    }
