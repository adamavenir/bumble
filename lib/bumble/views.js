...
Views.prototype.index = function (req, res) {
    ...
    // logger.info('howdy from views.index');

    //logger.info('index gravatar:', self.postData.gravatar);

    // logger.info(config.blogTitle);

    // logger.info('index self.postData:', self.postData);
...
Views.prototype.blogIndex = function (req, res) {
    ...
    // logger.info('howdy from views.blogIndex');

    // logger.info(config.blogTitle);
    paginator.paginate(this.postData, maxPosts, req, res, function (realPage) {
        if (realPage) return res.redirect('/?page=' + realPage);
